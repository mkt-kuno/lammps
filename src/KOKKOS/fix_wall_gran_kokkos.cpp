...
void FixWallGranKokkos<DeviceType>::post_force(int /*vflag*/)
{
  ...
  if (pairstyle == HOOKE)
    ...
    if (wallstyle == XPLANE)
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,TagFixWallGranHertzHistory<XPLANE>>(0,nlocal),*this);
    else if (wallstyle == YPLANE)
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,TagFixWallGranHertzHistory<YPLANE>>(0,nlocal),*this);
    else if (wallstyle == ZPLANE)
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,TagFixWallGranHertzHistory<ZPLANE>>(0,nlocal),*this);
    else if (wallstyle == ZCYLINDER)
      Kokkos::parallel_for(Kokkos::RangePolicy<DeviceType,TagFixWallGranHertzHistory<ZCYLINDER>>(0,nlocal),*this);
  } else {
...
template<int WallStyle>
KOKKOS_INLINE_FUNCTION
void FixWallGranKokkos<DeviceType>::operator()(TagFixWallGranHertzHistory<WallStyle>, const int &i) const
{
  double vwall_[3];
  vwall_[0] = vwall[0];
  vwall_[1] = vwall[1];
  ...
  if (mask[i] & groupbit) {
    ...
    if (rsq > radius*radius) {
      ...
      double damp = meff*gamman*vnnr*rsqinv;
      double ccel = kn*(radius-r)*rinv - damp;
      double polyhertz = sqrt((radius-r)*radius); //@note 2024.03.28 M.Kuno
      ccel *= polyhertz; //@note 2024.03.28 M.Kuno
      //if (limit_damping && (ccel < 0.0)) ccel = 0.0; //@note 2024.03.28 M.Kuno
      ...
      // tangential forces = shear + tangential velocity damping
      double fs1 = -polyhertz * (kt*d_history_one(i,0) + meff*gammat*vtr1);
      double fs2 = -polyhertz * (kt*d_history_one(i,1) + meff*gammat*vtr2);
      double fs3 = -polyhertz * (kt*d_history_one(i,2) + meff*gammat*vtr3);
