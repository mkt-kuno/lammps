  ...
  Dump(lmp, narg, arg)
{
  ...
  //~ Add the sorting information to the constructor
  sort_flag = 1;
  sortcol = 0; //~ Sort on atom ID

  //~ Set vtkflag to indicate that dump_vtk is active
  vtkflag = 1;

  //~ Initialise alreadywritten at 0
  alreadywritten = 0;
  ...
  //~ Test for 2D and issue an error if true (lazy programmer!)
  if (domain->dimension == 2)
    error->all(FLERR,"Dump vtk has not been implemented for 2D simulations");

  // setup column string

  /*~ This part required extensive modification. All of the labels 
    are now stored in columns, and 4 labels are automatically added
    at the start (x,y,z,diameter). It is also done at an earlier 
    point in the constructor to allow size_one to be calculated 
    correctly [KH - 5 May 2012]*/
  int colcounter = 4;

  for (int iarg = 5; iarg < narg; iarg++)
    if (strcmp(arg[iarg],"x") != 0 && strcmp(arg[iarg],"y") != 0 && strcmp(arg[iarg],"z") != 0 && strcmp(arg[iarg],"diameter") != 0) colcounter += 1; //~ Skip adding these twice
  
  columns = new char*[colcounter];
  columns[0] = new char[2];
  strcpy(columns[0],"x");

  columns[1] = new char[2];
  strcpy(columns[1],"y");

  columns[2] = new char[2];
  strcpy(columns[2],"z");

  columns[3] = new char[9];
  strcpy(columns[3],"diameter");

  colcounter = 4;
  for (int iarg = 5; iarg < narg; iarg++) {
    if (strcmp(arg[iarg],"x") != 0 && strcmp(arg[iarg],"y") != 0 && strcmp(arg[iarg],"z") != 0 && strcmp(arg[iarg],"diameter") != 0) {
      int leng = strlen(arg[iarg]) + 1;
      columns[colcounter] = new char[leng];
      strcpy(columns[colcounter],arg[iarg]);
      colcounter += 1;
    }
  }
  
  //~ Adjust size_one to keep this variable correct
  size_one = nfield = colcounter;
...
DumpVTK::~DumpVTK()
{
  ...
  //~ Add in the appropriate destructors
  for (int i = 0; i < size_one; i++) delete [] columns[i];
  delete [] columns;
}
...
void DumpVTK::init_style()
{
  ...
  //~ Exit if a binary file has been specified
  if (binary) error->all(FLERR,"Dump vtk is meaningless if binary files are written");
  
  // setup function ptrs
  header_choice = &DumpVTK::header_item;
  ...
  //~ Add in a check for appropriate specified run conditions
  if (multifile == 0) error->all(FLERR,"Dump vtk is useless unless a separate vtk file is written for each timestep");
  if (sort_flag == 0 || sortcol != 0) error->all(FLERR,"Dump vtk requires sorting by atom ID");
}

/* ---------------------------------------------------------------------- */

void DumpVTK::write_header(bigint ndump)
{
...
void DumpVTK::header_item(bigint ndump)
{
  //~ Set up the header for writing out the particle positions
...
{
  write_choice = &DumpVTK::write_text; //~ Added this line
  (this->*write_choice)(n,mybuf);
}

/* ---------------------------------------------------------------------- */

void DumpVTK::write_diameter(int n, double *mybuf)
{
  write_choice = &DumpVTK::write_diam;
  (this->*write_choice)(n,mybuf);
...
void DumpVTK::write_extra(int n, double *mybuf, int j)
{
  write_additional = &DumpVTK::write_supp;
  (this->*write_additional)(n,mybuf,j);
}
...
{
  //* Added this function to close the vtk files [KH - 31 May 2012]
...
void DumpVTK::write_text(int n, double *mybuf)
{
  int m = 0;
  alreadywritten = 0; //~ Reset to zero if non-zero

  /*~ First of all, write out the x,y and z positions of the particles
    as these are always needed in ParaView. The position of the first
    particle is stored in m = 0 (x), 1 (y) and 2 (z).*/
...
void DumpVTK::write_diam(int n, double *mybuf)
{
  //~ Now write out the particle diameters, stored in buffer position +3
  if (alreadywritten == 0) {
    fprintf(fp,"</DataArray>\n");
    fprintf(fp,"</Points>\n");
    fprintf(fp,"<PointData>\n");
    fprintf(fp,"<DataArray type=\"Float32\" Name=\"Diameter\" format=\"ascii\">\n");
    alreadywritten = 1;
  }
...
void DumpVTK::write_supp(int n, double *mybuf, int j)
{
  //~ Finally write out the other remaining desired variables
  if (j - alreadywritten == 3) {
    fprintf(fp,"</DataArray>\n");
    if (vtype[j] == INT) fprintf(fp,"<DataArray type=\"Int32\" Name=\"%s\" format=\"ascii\">\n",columns[j]);
    else if (vtype[j] == DOUBLE) fprintf(fp,"<DataArray type=\"Float32\" Name=\"%s\" format=\"ascii\">\n",columns[j]);
    else error->all(FLERR,"Invalid data type in write_supp");
    alreadywritten += 1;
  }
...
int DumpVTK::parse_fields(int narg, char **arg)
{
  ...
  int i;
   /*~ Note that the first four buffer slots (5-8) have been allocated to 
     x, y, z and diameter for ParaView purposes*/
  
  pack_choice[0] = &DumpVTK::pack_x;
  vtype[0] = DOUBLE;
   
  pack_choice[1] = &DumpVTK::pack_y;
  vtype[1] = DOUBLE;
  
  pack_choice[2] = &DumpVTK::pack_z;
  vtype[2] = DOUBLE;

  if (!atom->radius_flag)
    error->all(FLERR,"Dumping an atom property that isn't allocated");
  pack_choice[3] = &DumpVTK::pack_diameter;
  vtype[3] = DOUBLE;

  int extrasub = 0; //~ Define a separate quantity to subtract if x,y,z and diameter are ignored

  for (int iarg = 5; iarg < narg; iarg++) {
    i = iarg-1-extrasub; //~ Changed to 1
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      ...
      pack_choice[i] = &DumpVTK::pack_mass;
      vtype[i] = DOUBLE;
      ...
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_xs_triclinic;
      else pack_choice[i] = &DumpVTK::pack_xs;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ys") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_ys_triclinic;
      else pack_choice[i] = &DumpVTK::pack_ys;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zs") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_zs_triclinic;
      else pack_choice[i] = &DumpVTK::pack_zs;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_xu_triclinic;
      else pack_choice[i] = &DumpVTK::pack_xu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"yu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_yu_triclinic;
      else pack_choice[i] = &DumpVTK::pack_yu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_zu_triclinic;
      else pack_choice[i] = &DumpVTK::pack_zu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xsu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_xsu_triclinic;
      else pack_choice[i] = &DumpVTK::pack_xsu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ysu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_ysu_triclinic;
      else pack_choice[i] = &DumpVTK::pack_ysu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zsu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpVTK::pack_zsu_triclinic;
      else pack_choice[i] = &DumpVTK::pack_zsu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ix") == 0) {
      pack_choice[i] = &DumpVTK::pack_ix;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"iy") == 0) {
      pack_choice[i] = &DumpVTK::pack_iy;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"iz") == 0) {
      pack_choice[i] = &DumpVTK::pack_iz;
      vtype[i] = INT;

    } else if (strcmp(arg[iarg],"vx") == 0) {
      pack_choice[i] = &DumpVTK::pack_vx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"vy") == 0) {
      pack_choice[i] = &DumpVTK::pack_vy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"vz") == 0) {
      pack_choice[i] = &DumpVTK::pack_vz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fx") == 0) {
      pack_choice[i] = &DumpVTK::pack_fx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fy") == 0) {
      pack_choice[i] = &DumpVTK::pack_fy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fz") == 0) {
      pack_choice[i] = &DumpVTK::pack_fz;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"q") == 0) {
      if (!atom->q_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_q;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"mux") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_mux;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"muy") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_muy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"muz") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_muz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"mu") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_mu;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"radius") == 0) {
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_radius;
      vtype[i] = DOUBLE;
      ...
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_omegax;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegay") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_omegay;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegaz") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_omegaz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomx") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_angmomx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomy") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_angmomy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomz") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_angmomz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqx") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_tqx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqy") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_tqy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqz") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_tqz;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"spin") == 0) {
      if (!atom->spin_flag)
        error->all(FLERR,"Dumping an atom quantity that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_spin;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"eradius") == 0) {
      if (!atom->eradius_flag)
        error->all(FLERR,"Dumping an atom quantity that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_eradius;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ervel") == 0) {
      if (!atom->ervel_flag)
        error->all(FLERR,"Dumping an atom quantity that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_ervel;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"erforce") == 0) {
      if (!atom->erforce_flag)
        error->all(FLERR,"Dumping an atom quantity that isn't allocated");
      pack_choice[i] = &DumpVTK::pack_erforce;
      vtype[i] = DOUBLE;
    ...
    // if no trailing [], then arg is set to 0, else arg is int between []
      ...
      pack_choice[i] = &DumpVTK::pack_compute;
      vtype[i] = DOUBLE;
      ...
      if (argindex[i] == 0 && modify->compute[n]->size_peratom_cols > 0)
        error->all(FLERR,"Dump vtk compute does not calculate per-atom vector");
      if (argindex[i] > 0 && modify->compute[n]->size_peratom_cols == 0)
        error->all(FLERR,"Dump vtk compute does not calculate per-atom array");
      if (argindex[i] > 0 &&
    ...
    // if no trailing [], then arg is set to 0, else arg is between []
      ...
      pack_choice[i] = &DumpVTK::pack_fix;
      vtype[i] = DOUBLE;
    ...
    // variable value = v_name
      ...
      pack_choice[i] = &DumpVTK::pack_variable;
      vtype[i] = DOUBLE;
...
void DumpVTK::pack_molecule(int n)
{
  int *molecule = atom->molecule;
...
void DumpVTK::pack_xs_triclinic(int n)
{
  int j;
  double **x = atom->x;
...
void DumpVTK::pack_ys_triclinic(int n)
{
  int j;
  double **x = atom->x;
...
void DumpVTK::pack_zs_triclinic(int n)
{
  double **x = atom->x;
...
void DumpVTK::pack_xu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpVTK::pack_yu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpVTK::pack_zu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpVTK::pack_xsu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpVTK::pack_ysu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpVTK::pack_zsu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpVTK::pack_radius(int n)
{
  double *radius = atom->radius;
...
void DumpVTK::pack_diameter(int n)
{
  double *radius = atom->radius;
...
void DumpVTK::pack_omegax(int n)
{
...
void DumpVTK::pack_omegay(int n)
{
  double **omega = atom->omega;
...
void DumpVTK::pack_omegaz(int n)
{
  double **omega = atom->omega;
...
void DumpVTK::pack_angmomx(int n)
{
...
void DumpVTK::pack_angmomy(int n)
{
  double **angmom = atom->angmom;
...
void DumpVTK::pack_angmomz(int n)
{
  double **angmom = atom->angmom;
...
void DumpVTK::pack_eradius(int n)
{
...
void DumpVTK::pack_erforce(int n)
{
